...
class ImageDetector:
    def __init__(self, models_config=None):
        ...
        self.EXPECTED_ROOM_LENGTH = [3]  # ความยาวตัวเลขห้องที่คาดหวัง
        self.EXPECTED_METER_LENGTH = [4]  # ความยาวตัวเลขมิเตอร์ที่คาดหวัง
    ...
    def preprocess_for_ocr(self, img, class_name):
        ...
        if class_name == "meter" or class_name == "meter1" :
    ...
    def perform_ocr(self, img_name, img, class_name):
        """ทำ OCR กับภาพหนึ่งภาพ และคืนผลลัพธ์"""
        
        # ปรับพารามิเตอร์ EasyOCR หลายแบบ
        ocr_results = []
        
        # แบบที่ 1: ตั้งค่าพื้นฐานที่ปรับปรุงแล้ว Base Configuration
        try:
        ...
        # แบบที่ 2: เน้นการอ่านตัวเลขแบบแยกส่วน Segment Reading
        try:
            result2 = self.reader.readtext(
                ...
                text_threshold=0.1,
                low_text=0.2,
                mag_ratio=6.0 if class_name == "meter1" else 1.5  # ปรับ mag_ratio เฉพาะ decimal
            )
        ...
        if class_name == "meter1":
            # แบบที่ 3: การตั้งค่าสำหรับตัวเลขที่ชัดเจน Clear Reading
            try:
                result3 = self.reader.readtext(
                    ...
                    link_threshold=0.5,
                    canvas_size=1280,
                    mag_ratio=4.0 if class_name == "meter1" else 1.0,
                    slope_ths=0.2,
                    ycenter_ths=0.5,
                    height_ths=0.5,
                    width_ths=0.5,
            ...
            # แบบที่ 4: การตั้งค่าสำหรับตัวเลขที่จางหรือเบลอ Blur Reading
            try:
                result4 = self.reader.readtext(
                    ...
                    link_threshold=0.2,
                    canvas_size=3840,
                    mag_ratio=12.0 if class_name == "meter1" else 3.0,  # ปรับ mag_ratio เฉพาะ decimal
                    contrast_ths=0.05,
                    adjust_contrast=0.3,
                    height_ths=0.3,
                    width_ths=0.3,
    ...
    def filter_and_select_best_result(self, all_ocr_results, expected_lengths=None):
        """กรองและเลือกผลลัพธ์ OCR ที่ดีที่สุด"""
        if not all_ocr_results:
            return None

        # กรองผลลัพธ์เฉพาะความยาวที่คาดหวัง
        valid_results = []
        
        for text, conf, method in all_ocr_results:
            if text:  # ถ้ามีตัวเลข
                # ตรวจสอบความยาวตามที่กำหนดไว้อย่างเข้มงวด
                if expected_lengths and len(text) in expected_lengths:
                    valid_results.append((text, conf, method))
        
        # ถ้าไม่มีผลลัพธ์ที่ตรงตามความยาวที่กำหนด ให้ return None
        if not valid_results:
            print(f"ไม่พบผลลัพธ์ที่มีความยาวตามที่คาดหวัง: {expected_lengths}")
            return None
        
        # จัดกลุ่มผลลัพธ์ตามตัวเลข
        text_frequency = {}
            ...
            if conf >= self.CONF_THRESHOLD:
                ...
                if class_name:
                    ...
                    with ThreadPoolExecutor(max_workers=4) as executor:
                        futures = [executor.submit(self.perform_ocr, img_name, img, class_name) 
    ...
    def find_best_pairs_unified(self, detections_by_class):
        """
        หาคู่ที่ดีที่สุด - จะ return ผลลัพธ์เสมอ
        ถ้าไม่พบคู่ที่มีทั้งเลขห้องและมิเตอร์ จะเลือกผลลัพธ์ที่ดีที่สุดที่มี
        """
        ...
        if room_detections and meter_detections:
            ...
            for room in room_detections:
                for meter in meter_detections:
                    ...
                    if closest_decimal and min_decimal_distance < 200:
        ...
        # กรณีที่ไม่มีคู่ที่สมบูรณ์ - เลือกผลลัพธ์ที่ดีที่สุดที่มี
        result = {
            'room': None,
            'meter': None,
            'decimal': None,
            'distance': None,
            'score': 0,
            'pairing_method': 'best_individual'
        }
        ...
        if meter_detections:
            ...
            if decimal_detections:
                ...
                # เก็บเลขทศนิยมที่ใกล้ที่สุด (ถ้าอยู่ใกล้พอ)
                if closest_decimal and min_decimal_distance < 600:
                    result['decimal'] = closest_decimal
                    result['distance'] = min_decimal_distance
                    result['score'] += closest_decimal['confidence'] * 0.1
        
        # กรณีที่มีแต่เลขทศนิยม (ไม่มีมิเตอร์)
        elif decimal_detections and not meter_detections:
            best_decimal = max(decimal_detections, 
                            key=lambda x: x['confidence'] * 0.7 + x['detection_confidence'] * 0.3)
            result['decimal'] = best_decimal
            result['score'] += best_decimal['confidence'] * 0.2
        
        # return ผลลัพธ์ที่มี (แม้ว่าจะไม่ครบถ้วน)
        if result['room'] or result['meter'] or result['decimal']:
            return result
        
        return None

    def draw_detection_results_unified(self, img, best_pair):
        """
        วาด bounding box และข้อความผลลัพธ์บนรูปภาพ
        """
    ...
    def process_image(self, image_path, output_folder):
        ...
        if best_pair['pairing_method'] == 'proximity_matching':
            ...
            if best_pair['meter']:
                ...
                if best_pair['decimal']:
                    ...
                    full_meter = f"{best_meter['number']}.{best_decimal['number']}"
                    print(f"   พบเลขมิเตอร์: {best_meter['number']} พร้อมทศนิยม: {best_decimal['number']}")
                else:
        ...
        display_img = self.draw_detection_results_unified(img, best_pair)
